...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  1 16:28:59 2020
...
pairs = ['EUR/USD','GBP/USD','AUD/USD','USD/CAD',
         'USD/CHF','EUR/GBP'] #currency pairs to be included in the strategy
#pairs = ['EUR/USD','GBP/USD','AUD/USD','USD/CAD','USOil','SPX500', 'JPN225','USEquities',
         #'CHN.ECOMM','CHN.TECH','FAANG','CANNABIS','BIOTECH','ESPORTS','XLM/USD','EOS/USD',
         #'CryptoMajor','XRP/USD','LTC/USD','ETH/USD','BCH/USD','BTC/USD','JPYBasket','EMBasket',
         #'EUR/JPY','USD/JPY','AUD/JPY','AUD/NZD','NZD/USD','USD/CHF','CORNF','SOYF','EUR/GBP',
         #'GER30','XAU/USD']
...
def renko_merge(DF):
    ...
    merged_df["macd"]= MACD(merged_df,12,24,6)[0]
    merged_df["macd_sig"]= MACD(merged_df,12,24,6)[1]
...
def trade_signal(MERGED_DF,l_s):
    "Data processing function to generate signal - this is what determines a trade"
    signal = ""
        ...
        for currency in pairs:
            ...
            if signal == "Buy":
                con.open_trade(symbol=currency, is_buy=True, is_in_pips=True, amount=pos_size,
                               time_in_force='GTC', stop=-10, trailing_step =True, order_type='AtMarket')
