...
# -*- coding: utf-8 -*-
"""
Created on Thu May 28 17:01:06 2020
        ...
        if tickers_signal[ticker] == "":
            ...
            if i > 0:
                if ohlc_renko[ticker]["bar_num"][i]>=2 and ohlc_renko[ticker]["macd"][i]>ohlc_renko[ticker]["macd_sig"][i] and ohlc_renko[ticker]["macd_slope"][i]>ohlc_renko[ticker]["macd_sig_slope"][i]:
                    #Buy/Long -> Renko bar >= 2 AND macd line above signal line AND macd_slope > signal_slope -> buy
                    ...
                    #Sell/Short -> Renko bar <= -2 AND macd line below signal line AND macd_slope < signal_slope -> sell
                    tickers_signal[ticker] = "Sell"

        elif tickers_signal[ticker] == "Buy":
            tickers_ret[ticker].append((ohlc_renko[ticker]["Adj Close"][i]/ohlc_renko[ticker]["Adj Close"][i-1])-1)
            #Calculates return from ratio of close and previous close
            if i > 0:
                if ohlc_renko[ticker]["bar_num"][i]<=-2 and ohlc_renko[ticker]["macd"][i]<ohlc_renko[ticker]["macd_sig"][i] and ohlc_renko[ticker]["macd_slope"][i]<ohlc_renko[ticker]["macd_sig_slope"][i]:
                    tickers_signal[ticker] = "Sell"
                    #If above is true, signal becomes 'Sell'
                    ...
                    tickers_signal[ticker] = ""
                    #Above exit criteria does not take renko into account
            #Equal and opposite for 'Sell'
